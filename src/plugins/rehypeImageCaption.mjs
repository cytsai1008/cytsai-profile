    ...
    visit(tree, (node, parent, index) => {
      ...
      parent.children[index] = {
        ...
        children: [
          styledImageNode,
          {
            // The caption text IS the image's alt, so screen readers already announce
            // it via the img. aria-hidden keeps it visible without announcing it twice.
            // (The alt itself can't be emptied here — astro:assets re-renders the img
            // from the original markdown after this plugin runs.)
            type: "element",
            tagName: "figcaption",
            properties: {
              "aria-hidden": "true",
