...
interface Props {
  ...
  locale: Locale;
  currentPath: string;
  themeLabels: { label: string; light: string; system: string; dark: string };
...
const themeOptions: { mode: ThemeMode; icon: LucideIcon; label: string }[] = [
  { mode: "light", icon: Sun, label: props.themeLabels.light },
  { mode: "system", icon: Monitor, label: props.themeLabels.system },
  { mode: "dark", icon: Moon, label: props.themeLabels.dark },
];

const activeTheme = computed(() => themeOptions.find((o) => o.mode === theme.value)!);

const notoFontClass = computed(() =>
  props.locale === "zh-tw"
    ? "font-noto-tc"
    : props.locale === "zh-cn"
      ? "font-noto-sc"
...
<template>
  <div data-navbar class="sticky top-0 z-50">
    <header class="w-full border-b border-border bg-surface/60 backdrop-blur-3xl transition-colors">
      ...
      >
        <!-- Col 1: empty spacer — keeps the grid's center column truly centered -->
        <div aria-hidden="true" class="md:justify-self-start" />
        ...
        <ul class="m-0 hidden list-none items-center gap-1 p-0 md:flex">
          <li v-for="link in links" :key="link.href">
            <a
              ...
              class="rounded-lg px-3 py-1.5 text-sm font-medium capitalize no-underline transition-colors"
              :class="[
                notoFontClass,
                isActive(link.href)
        ...
        <div class="js-only flex items-center justify-end gap-1">
          ...
          >
            ...
            <button
              ref="themeCompact"
              class="relative rounded-lg p-1.5 text-fg-muted transition-colors before:absolute before:-inset-2 hover:bg-surface-alt hover:text-fg"
              :aria-label="`${themeLabels.label}: ${activeTheme.label}`"
              @click="cycleTheme"
            >
            ...
            >
              <button
                ...
                :key="opt.mode"
                class="relative p-1.5 transition-colors before:absolute before:inset-x-0 before:-inset-y-2 first:rounded-l-lg last:rounded-r-lg"
                :class="
                  theme === opt.mode
          ...
          <div class="relative" @mouseenter="showLangText" @mouseleave="hideLangText">
            <button
              ref="langBtnRef"
              class="relative flex items-center gap-1 rounded-lg px-2.5 py-1.5 text-sm font-medium text-fg transition-colors before:absolute before:inset-x-0 before:-inset-y-2 hover:bg-surface-alt"
          ...
          <button
            ref="hamburgerRef"
            class="relative rounded-lg p-2 text-fg transition-colors before:absolute before:-inset-1 hover:bg-surface-alt md:hidden"
      ...
      <ul class="m-0 flex list-none flex-col gap-1 p-0 pt-3">
        <li v-for="(link, i) in links" :key="link.href">
          <a
            ...
            :href="link.href"
            :aria-current="isActive(link.href) ? 'page' : undefined"
            class="block rounded-lg px-3 py-2 text-sm font-medium capitalize no-underline transition-colors"
            :class="[
              notoFontClass,
              isActive(link.href)
                ? 'bg-accent-bg hover:bg-accent-dim'
